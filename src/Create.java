    ...
    public static void create() {
        try {
            ...
            if (!databaseExists)
            {
                ...
                stmt.executeUpdate("CREATE TABLE BOOK_LOANS(LOAN_ID INT NOT NULL  PRIMARY KEY,ISBN VARCHAR(255),CARDID INT,RETURN_DATE VARCHAR(20),DUE_DATE VARCHAR(50),ISSUED_DATE VARCHAR(20),FOREIGN KEY (ISBN) REFERENCES BOOK(ISBN),FOREIGN KEY (CARDID) REFERENCES BORROWER(CARDID))");
                //CREATE TABLE FINES
                stmt.executeUpdate("CREATE TABLE FINES(LOAN_ID INT NOT NULL  PRIMARY KEY,FINE_AMOUNT INT,PAID INT,FOREIGN KEY (LOAN_ID) REFERENCES BOOK_LOANS(LOAN_ID))");
                resultSet.close();
            }
        }
